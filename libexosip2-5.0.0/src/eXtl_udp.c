...
udp_tl_read_message (struct eXosip_t *excontext, fd_set * osip_fdset, fd_set * osip_wrset)
{
  struct eXtludp *reserved = (struct eXtludp *) excontext->eXtludp_reserved;
  socklen_t slen;
  int i;  //why not init it when use it, hart to read
  ...
  if (reserved->udp_socket <= 0)  //udp_socket是接收套接口
    return -1;
...
static struct eXtl_protocol eXtl_udp = {
  ...
  &udp_tl_init,   //分配一个struct eXtludp *，存入eXosip_t->eXtludp_reserved
  &udp_tl_free,
  &udp_tl_open,   //创建接收套接口，启动监听，将套接口存到eXosip_t->udp_socket中
  &udp_tl_set_fdset,
  &udp_tl_read_message,
  &udp_tl_send_message,
  &udp_tl_keepalive,
